...
const emit = defineEmits<{
  close: []
  submit: [data: { title: string; description: string; priorityId: number; highPriorityReason: string; attachments: File[] }]
  'status-changed': []
}>()
...
const isFormValid = computed(() => {
  ...
  if (isHighPriority.value) {
    return titleValid && descValid && priorityValid && form.highPriorityReason.trim().length > 0
  }
...
function fillForm() {
  if (props.ticket) {
    ...
    form.description = props.ticket.messages[0]?.content || ''
        ...
        <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-hidden z-10">
          ...
          <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            <div>
              ...
              <p class="text-xs text-gray-400 font-mono mt-0.5">{{ ticket.code }}</p>
            </div>
            <button
              @click="handleClose"
          ...
          <div class="px-6 py-5 overflow-y-auto max-h-[calc(90vh-140px)]">
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">
                Attachments <span class="text-gray-400 text-xs">(kosongkan jika tidak ingin mengubah)</span>
              </label>
